...
exports.byValue = exports.byName = undefined;
exports.parse = parse;
...
var NETWORK_LOG_PATTERN = /^(\S+)(?:\s+)(\d+):(\d+)(.*)$/i;
var CONSOLE_LOG_PATTERN = /^(?:console-api\s+)(\d+)\:(\d+)(?:\s+)(.*)$/i;
...
// CSS related
'Declaration dropped', 'Ruleset ignored due to bad selector', 'Expected declaration but found', 'Expected media feature name but found', 'Unrecognized at-rule', 'Keyframe rule ignored due to bad selector', 'Invalid CSS property name', 'Invalid CSS property value', 'Invalid CSS media query',
// addons stuff
'Could not read chrome manifest', 'blocklist is disabled', 'Trying to re-register CID', 'chrome-extension://', 'resource://', 'Native module at path', 'Failed to load native module at path', 'Component returned failure code', 'While registering XPCOM module', 'to preference extensions', 'addons.xpi:',
// just useless here
 ...
 * @return {Boolean}
 */
var insideIgnoredLogs = (0, _ramda.compose)((0, _ramda.flip)(_ramda.any)(IGNORED_LOGS), (0, _ramda.flip)(_ramda.contains));
...
function parse(log) {
  ...
  var addonLog = log.message.match(FIREFOX_ADDON_LOG_PATTERN);
  if (addonLog) {
    return {
      addon: true,
  ...
  var consoleLogMessage = log.message.match(CONSOLE_LOG_PATTERN);
  ...
  // parse network logs
  var networkLogMessage = log.message.match(NETWORK_LOG_PATTERN);
  ...
  var chromeLogMessage = log.message.match(CHROME_LOG_PATTERN);
  if (chromeLogMessage) {
    return {
  ...
  if (log.message.match(ANDROID_EMULATOR_LOG_PATTERN)) {
    var androidEmulatorBrowserMessage = log.message.match(ANDROID_EMULATOR_BROWSER_MESSAGE_PATTERN);
  ...
  if (parsed && (0, _ramda.is)(Object, parsed.message)) {
    return {
 ...
 */
var byValue = exports.byValue = (0, _ramda.invertObj)(byName);
