 ...
 * @property {Function|Array} sequence[n] - function that takes previous chain element as argument
 *                         and should return promise chained to this | array of functions to process
 *                         resursivelly
 ...
 * });
 */
export function chain(prev, sequence) {
    let results = [ ];
    return sequence.reduce(function(prev, curr) {
        return Array.isArray(curr) ? chain(prev, curr).then((result) => {
            results.push(result);
